...
5. 跑完全部发版前回归。
   ...
   `test:e2e` 有失败用例时，按**修复循环**处理（不要每改一次就重跑全量）：

   1. 定位失败根因后逐个（或按同根因批次）修复。
   2. 只重跑刚修的那些用例，不跑全量——改过代码后先重新 build，再按测试名定点：
      ...
      `--grep` 按测试名正则匹配，可并列多个（如 `"A|B|C"`）。

   3. 全部失败修复并各自定点验证通过后，再跑一次完整 `bun run test:e2e`，
      确认没有修复引入的新回归且全部通过。
   4. 若全量又出现新失败（修复引入的），回到第 1 步继续循环，直到全量全绿。
