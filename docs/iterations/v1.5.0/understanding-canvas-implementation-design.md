...
└── 主区：画布工作区
     ├── 顶部工具栏（精简）：画布标题、理解库（开右侧面板）、文本卡（可拖拽 icon）、撤销 / 重做、适应视图
     ├── X6 画布（无限画布、点状网格、平移缩放、参考线、框选多选、键盘）
     ├── 右侧理解库面板（「理解库」按钮打开：domain filter + 搜索 + 排序 + 理解列表，卡片拖入画布）
     └── 右侧理解详情面板（点击理解卡打开，复用 UnderstandingDetail，可关闭；与理解库面板的并存方式后议）
```
...
| F4  | 新建文本卡          | 工具栏「文本卡」icon button，**拖拽到画布落点创建**（点击行为后议）       | 创建后可编辑文本                                     |
...
| F10 | 撤销 / 重做         | X6 History：位置、增删、标签、文本                                        | Ctrl/Cmd+Z 与工具栏按钮均可用                        |
| F11 | 框选多选 / 整组拖动 | 橡皮筋框选、Shift 多选、整体移动                                          | 多选移动后各自位置持久化                             |
| F12 | 删除                | 选中卡片 / 连线后 Delete / Backspace（确认）                              | 级联删除连线                                         |
| F13 | 打开理解详情        | 点击理解卡 → 右侧打开 UnderstandingDetail，编辑在详情内                   | 详情可编辑、保存后画布卡片内容同步                   |
| F14 | 持久化与恢复        | 位置 / 尺寸 / 文本 / 连线 / 标签落库；视口（缩放平移）恢复                | 重进画布完整还原                                     |

### 1.4 待后续讨论（本轮不定）

- 理解详情面板与理解库面板的并存 / 切换方式。
- 「文本卡」icon button 的点击行为（是否支持点击在画布中心创建，还是仅拖拽）。
- 工具栏是否继续精简（撤销 / 重做、适应视图的去留与位置）。
...
CREATE TABLE understanding_canvas_elements (
  ...
  kind             TEXT NOT NULL CHECK (kind IN ('understanding', 'text')),
  understanding_id TEXT REFERENCES understandings(id) ON DELETE SET NULL, -- 被删理解置空 → 占位
  text             TEXT,              -- 文本卡内容（Markdown）
  x                REAL NOT NULL,     -- X6 模型坐标
  y                REAL NOT NULL,
  width            REAL NOT NULL,
...
| 从面板拖入 / 新建                | `createElement`（kind + understandingId / text + x / y + 尺寸） |
...
| DnD 拖入落点                       | `onElementDropped({ kind, understandingId?, text?, x, y })` | `createElement`                |
...
- X6 3.x：插件在核心包（`Dnd / Snapline / Selection / Keyboard / History`），需显式 `graph.use(new History({ enabled: true }))`；`dnd.start()` 传 `graph.createNode()` 实例；运行时依赖 `tslib` 需加入 `packages/ui`。
- 新元素在 DnD 时先用服务端生成的 id 建好 node data（`{ id, kind, understandingId?, text }`），落点后统一走 `createElement`，避免双 id 问题。
...
type UnderstandingCanvasStore = {
  ...
  detailPanelOpen: boolean; // 理解详情面板
};
```
...
apps/electron/src/renderer/src/modules/understanding-canvas/
  ...
  canvas/
    ...
    CanvasToolbar.tsx            # 标题、理解库（开右侧面板）、文本卡（可拖拽 icon）、撤销 / 重做、缩放
    LibraryPanel.tsx             # 理解库面板：复用 understanding-list 的 domain filter / 搜索 / 排序能力，主动作是拖入画布
    UnderstandingDetailPanel.tsx # 复用 UnderstandingDetail（右侧）
  adapters/
