...
describe("全局行为", () => {
  ...
  describe("修改确认", () => {
    it("修改命令未加 --yes 时拒绝执行", async () => {
      ...
      const { code: code2, stderr: stderr2 } = await runCommand(["category", "delete", catId]);
      expect(code2).toBe(3);
    ...
    it("修改命令加 --yes 后正常执行", async () => {
      const { stdout } = await runCommand(["category", "list", "--format", "json"]);
      const arr = parseJson(stdout) as unknown[];
      const catId = (arr[0] as { id: string }).id;
  ...
  describe("参数校验", () => {
    it("整数选项传入非数字值时报错", async () => {
      const { code, stderr } = await runCommand(["thought", "list", "--limit", "not-a-number"]);
      expect(code).toBe(2);
      const err = JSON.parse(stderr);
    ...
    it("缺少子命令时报错", async () => {
      const { code, stderr } = await runCommand(["thought"]);
      expect(code).toBe(2);
      const err = JSON.parse(stderr);
      expect(err.code).toBe("VALIDATION_ERROR");
    });
