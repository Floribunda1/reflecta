 ...
 *   双指滚动 / 滚轮     → 平移
 *   捏合 / Cmd(⌘)+滚动  → 缩放（光标处为锚点）
 *   光标在已选中卡片内 → 滚卡片 overflow（滚到头也不平移画布）
...
/** 已选中卡片内的 overflow 容器；找不到则画布应接管滚轮。 */
function selectedCardScroller(target: EventTarget | null): HTMLElement | null {
  if (!(target instanceof Element)) return null;
  const card = target.closest(SELECTED_CARD);
  if (!(card instanceof HTMLElement)) return null;
  return card.querySelector<HTMLElement>(INNER_SCROLL);
}
...
export function applyWheelToInnerScroll(
  ...
  const scroller = selectedCardScroller(event.target ?? null);
