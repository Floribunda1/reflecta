...
Agent-Friendly CLI 应当遵循 Unix 哲学：每个命令只做一件事，并把这件事做好。输出是输入，命令之间可以通过管道自由组合。

- **单一职责**：每个子命令有清晰、聚焦的职责，不要试图在一个命令里完成「查询 + 分析 + 格式化」。
- **输出即输入**：stdout 的结构化输出可以被下一个命令直接消费（例如 `mycli task list --status pending | mycli task archive --stdin`）。
- **文本流作为通用接口**：优先使用结构化文本（JSON / JSONL）作为命令间的数据交换格式，而不是二进制或专有协议。
- **让 Agent 做上层组合**：CLI 提供原子能力，复杂工作流由 Agent 通过多命令组合或脚本来实现，而不是在 CLI 内部堆砌高层抽象。

这保证了 CLI 的可组合性和可预测性：Agent 知道每个命令的输入输出契约，就能像拼乐高一样自由组合它们。

### 2.7 最小化 Round Trip

Agent 的每次 Shell 调用都有延迟和 Token 开销。CLI 设计应尽量减少 Agent 与系统之间的往返次数。

- **批量操作优先**：提供 `batch create`、`batch update` 等命令，让 Agent 一次提交多条记录，而不是循环调用 N 次单条命令。
- **按需返回**：通过 `--fields` 或 `--include-*` 选项让 Agent 精确指定需要的数据，避免传输无用字段。
- **工作流 API**：如 3.4 所述，当 Agent 的常见任务天然需要一组相关对象时，提供聚合读取接口（如 `project snapshot`、`graph neighborhood`），避免 N+1 查询。
- **避免 Chatty 协议**：CLI 不实现分页游标（cursor）以外的增量拉取协议；Agent 应通过合理的 `--limit` 和 `--offset` 一次性获取所需数据。
