 ...
 * - 收起/展开直接 resize 到目标宽度，避免补间期间逐帧触发主内容重排。
 * - 收起后 visibility:hidden，保证菜单不可聚焦并保住 e2e toBeHidden 契约。
 */
...
function AppShell() {
  ...
  // 与 reduced-motion 一致：open 变化时直接落到目标宽度。
  useEffect(() => {
    const panel = railPanelRef.current;
    if (!panel) return;
    panel.resize(open ? lastOpenWidthRef.current : 0);
  }, [open]);
