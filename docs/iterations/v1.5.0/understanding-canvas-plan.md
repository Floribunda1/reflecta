...
| 理解卡           | 引用理解库的 Understanding | 显示标题 + 摘要，点击打开详情；一张理解可进多个画布（引用非副本） |
| 文本卡           | 画布内自由文本             | 放临时概念、备注、未沉淀的想法                                    |
...
5. 编辑体验：移动卡片（参考线对齐）、框选多选、撤销重做、删除（确认）。
6. 回到原文：点击理解卡 → 打开理解详情（交互方式见 §7 开放问题）。
7. 持久化：位置 / 文本 / 连线 / 标签落库，视口（缩放平移）恢复。
...
1. 理解卡点击行为：画布内打开详情（右侧滑出面板，不离开画布语境）还是跳转 Capture？
2. 卡片显示内容：标题 + 摘要截断（多少字）？是否带领域标签 / 上下文数等轻量信息？
3. 理解卡可编辑性：只读展示（详情去 Capture 改），还是画布上直接改标题 / 正文并同步回理解库？
4. 文本卡能力边界：纯文本即可，还是需要简单富文本（加粗 / 列表）？
5. 画布列表形态：纯列表是否够用？是否需要排序 / 分组？
6. 最终入口：正式名「理解画布」已确认，入口由用户设计。
