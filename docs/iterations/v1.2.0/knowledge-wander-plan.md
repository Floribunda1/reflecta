...
- 每条 Understanding 对应一个紧凑卡片节点。
- 每条 Connection 只有在两端节点都处于当前范围时才显示。
- 孤立 Understanding 仍显示，不增加「未连接」警告样式。
...
使用 [`@antv/g6`](https://github.com/antvis/G6) v5 作为图谱的单一引擎，负责节点和边渲染、布局、交互、缩放平移与 MiniMap。React 只负责容器、右侧详情和 design-system 控件，不再保留 React Flow 适配层。
...
使用 G6 内置 `d3-force`，不直接安装 `d3-force`：

- `link` 让真实 Connection 参与布局。
- `manyBody` 拉开节点，`collide` 按紧凑卡片的包围半径避免重叠。
- `x` / `y` 将包括孤立节点在内的所有节点轻量拉回画布中心。
- 使用正常模式，不按 Domain 或其他系统属性聚类。
- 布局参数集中在图谱模块，以代表性 fixture 调整，不暴露给用户。
- 节点拖动使用 G6 专用于 D3 Force 的 `drag-element-force` behavior，并在拖动结束后固定节点。
...
2. 动态加载 G6，并用 G6 behavior、viewport API 与 MiniMap plugin 提供画布交互。
3. 用 G6 内置 `d3-force` 替换 Dagre、React Flow 和 Domain lane。
4. 点击节点打开同一个详情 panel；选择节点突出一跳关系。
5. 缓存本次会话内稳定位置和 viewport，详情开关不得重新模拟。

完成条件：图谱包含范围内全部 Understanding 和真实 Connection，孤立节点可见，节点稳定且不持续漂移。
