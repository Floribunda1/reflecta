...
当我已经在多个领域沉淀了一批理解，
但回看时觉得它们像散乱条目，
我想知道哪些理解已经连起来、哪些仍然孤立、哪些缺少上下文、哪些跨到了其他领域，
以便决定下一步补上下文、补连接，还是沉淀更高层理解。
```
...
例如“呈现缺少 Context 的 Understanding”是 requirement；“节点用空心样式表示无上下文”才是某种 shape 下的 feature / shape rule。
...
| ------------------------- | ---------------------------------------------- | ---------------------------------------- |
| 单画布 Understanding 图谱 | 用节点和边直接展示 Understanding 与 Connection | 全局节点过多时可能变乱                   |
| 多栏 Topic Map            | 用领域、主题、Understanding 分层表达结构       | 主对象过多，新用户第一眼复杂             |
...
```text
第一版选择单画布 Understanding 图谱。

原因：
- 它最直接呈现 Understanding 与 Connection。
- 用户熟悉节点和边的图谱心智。
- Context 可以作为节点状态和点击详情承载。
- 孤岛可以自然表现为未连接节点。
...
```text
Selected Shape: 单画布 Understanding 图谱
...
| 节点上下文状态       | Core           | 呈现缺少 Context 的理解 | 判断理解是否有根              | 承载 Reflecta 的 Context 价值 |
...
| 打开原 Understanding | Support        | 从观察回到整理动作      | 回到原 Understanding 继续整理 | Graph 不是沉淀终点            |
...
- 理解深浅感 -> 节点上下文状态、连接状态、点击详情。
- Context 厚度 -> 上下文状态、上下文摘要、打开原 Understanding 补充入口。
...
| Shape Rule                    | 原因                                                |
| ----------------------------- | --------------------------------------------------- |
| Context 不作为一级节点        | Context 是理解的根，不是独立知识对象                |
