...
beforeEach(async () => {
  ...
  canvasId = (await service.createCanvas({ title: "灌溉调度" })).id;
});
...
describe("CanvasCore.searchCanvases (canvas_search 语义)", () => {
  test("matches canvas title (case-insensitive)", async () => {
    const hits = await service.searchCanvases({ query: "灌溉" });
  ...
  test("matches text card content", async () => {
    ...
    const hits = await service.searchCanvases({ query: "主管压力" });
    expect(hits.map((h) => h.canvas.id)).toEqual([canvasId]);
  ...
  test("matches edge labels", async () => {
    ...
    const hits = await service.searchCanvases({ query: "依赖" });
    expect(hits.map((h) => h.canvas.id)).toEqual([canvasId]);
  ...
  test("matches group label", async () => {
    ...
    const hits = await service.searchCanvases({ query: "闭环" });
    expect(hits.map((h) => h.canvas.id)).toEqual([canvasId]);
  ...
  test("matches referenced understanding title", async () => {
    ...
    const hits = await service.searchCanvases({ query: "压力先稳" });
    expect(hits.map((h) => h.canvas.id)).toEqual([canvasId]);
  ...
  test("multi-word OR semantics: any term matches", async () => {
    ...
    const hits = await service.searchCanvases({ query: "不存在的词 排水阀" });
    expect(hits.map((h) => h.canvas.id)).toEqual([canvasId]);
  });

  test("no match returns empty", async () => {
    const hits = await service.searchCanvases({ query: "完全不存在的词" });
    expect(hits).toEqual([]);
  });

  test("empty query returns empty", async () => {
    expect(await service.searchCanvases({})).toEqual([]);
    expect(await service.searchCanvases({ query: "   " })).toEqual([]);
  });
  ...
  test("understandingId reverse query returns canvas with reason", async () => {
    ...
    const hits = await service.searchCanvases({ understandingId: understanding.id });
    expect(hits.map((h) => h.canvas.id)).toEqual([canvasId]);
    expect(hits[0].snippet).toContain("逆查询理解");
  });

  test("listCanvases filters by title keyword and limit", async () => {
    const other = (await service.createCanvas({ title: "无关键词画布" })).id;
    const all = await service.listCanvases({ titleSearchKeyword: "灌溉" });
    expect(all.map((c) => c.id)).toEqual([canvasId]);
    const limited = await service.listCanvases({ limit: 1 });
