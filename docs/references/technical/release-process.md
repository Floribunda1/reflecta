...
5. 跑完全部发版前回归。
   ...
   `test:e2e` 有失败用例时，先区分 **flaky 与真实失败**，再按**修复循环**处理
   （不要每改一次就重跑全量）：

   1. 失败用例先定点重跑一次确认是否稳定复现（不修任何代码，直接按测试名定点）：

      ```bash
      bunx playwright test --grep "失败用例的测试名"
      ```

      重跑即通过 → 判定为 flaky（如并发导致的不稳定），记录在案即可，**不需要修复、
      不阻塞发版**。`--grep` 按测试名正则匹配，可并列多个（如 `"A|B|C"`）。

   2. 只有稳定复现的失败才是要修复的：定位失败根因后逐个（或按同根因批次）修复。
   3. 修完后只重跑刚修的那些用例，不跑全量——改过代码后先重新 build，再按测试名定点：
   ...
   4. 全部失败修复并各自定点验证通过后，再跑一次完整 `bun run test:e2e`，
      确认没有修复引入的新回归且全部通过。
   5. 若全量又出现新失败（修复引入的），回到第 1 步继续循环，直到全量全绿。
