...
| F24 | 连线样式            | 选中连线可配置：**拐点类型**（直线 / 贝塞尔弧线 / 正交直角）、**线型**（实线 / 虚线 / 点线）、**颜色**（预设色板）、**粗细**（细 / 中 / 粗）、箭头（默认有向，可去掉）；可重置默认；样式持久化 | 样式生效并随画布恢复，可重置                         |
...
type CanvasDocument = {
  ...
  // EdgeStyle 见服务端设计：{ routing?, lineStyle?, color?, width?, arrowhead? }
};
...
| 连线样式变更（选中后编辑）              | `onEdgeStyleChanged(id, style)`                                         | `updateEdge`                   |
...
- **连线样式（F24）X6 映射**：`straight → connector: normal`、`curve → connector: curve`、`orthogonal → router: orth + connector: rounded`（圆角 radius 可调）；线型用 `attrs.line.strokeDasharray`（虚线 / 点线）；颜色 / 粗细用 `attrs.line.stroke / strokeWidth`；箭头用 `attrs.line.targetMarker`（arrow / block / none）。样式变更监听 `edge:change:attrs` 落库（防抖）。
