...
test("@AG-CONV-006 用户在长对话中通过右侧摘录跳转到指定消息", async () => {
  ...
  try {
    ...
    await expect(jumpTrigger).toHaveAttribute("aria-label", /打开对话轮次导航.*共 6 轮/);
    await expect(jumpTrigger).toHaveCSS("opacity", "1");
    await expect(page.getByTestId("agent-chat-jump-position")).toContainText("/6");
    ...
    expect(jumpTriggerBounds.y).toBeLessThan(200);
...
test("@AG-CONTEXT-007 用户发送可读附件后看到 Agent 使用附件", async () => {
  ...
  try {
    ...
    await expect(attachmentTool.getByTestId("agent-tool-detail")).not.toBeVisible();
    await attachmentTool.click();
    await expect(attachmentTool.getByTestId("agent-tool-detail")).toBeVisible();
...
test("@AG-RETRIEVAL-003 用户要求 Agent 检索知识库后看到检索结果", async () => {
  ...
  try {
    ...
    await waitForAssistantReply(page);
    // 模型可能产生多组活动（检索后按工具指引读取候选理解）：展开包含首个 tool 活动的 group
    const toolActivity = page.getByTestId("agent-tool-activity").first();
    const groups = page.getByTestId("agent-activity-group");
    for (let i = (await groups.count()) - 1; i >= 0; i--) {
      await groups.nth(i).getByTestId("agent-activity-group-trigger").click();
      if (await toolActivity.isVisible()) break;
    }
