...
- Agent 默认语言从 `knowledge base` 转成 `personal understanding` / `Thought` / `Context provenance`。
- 读工具主入口从 `search_all` 转成产品语义的 `retrieve_understanding`。
- 写工具从“创建一个 Thought”转成“提交一个可追溯的理解候选”。
- Context 不再被描述成 surrounding material，而是 Thought 的来源证据。
...
- Agent 读取 Thought 时，应该优先关心它的 Context provenance。
- Agent 搜索 Context 时，应该把命中的 Context 作为父 Thought 的来源证据，而不是孤立材料。
- 没有 Context 的 Thought 可以存在，但 Agent 必须把它当成边界或未充分追溯的理解。
...
```txt
search_all -> thought_get(includeContexts) -> graph_neighborhood
```

这个 interface 太浅。复杂度暴露给了模型。

### 3.3 Search 返回 shape 不符合 Context provenance
...
这把 Thought 命中和 Context 命中拆成了两个列表。

但 Reflecta 的产品语义里，Context 命中不应该是另一类“搜索结果”。它应该被解释成：

```txt
某个 Thought 被它的来源证据命中了。
```
...
type UnderstandingCandidate = {
  ...
  matchedBy: "thought" | "context" | "connection";
  sourceEvidence: ContextEvidence[];
...
### 3.4 `thought_create` 允许 source-less AI proposal

当前 `thought_create` 参数是：
...
这个 shape 很容易让 Agent 做出一条漂亮但悬空的 Thought。
...
- Thought / Context / Connection / Category 的定义。
- 用户是大脑，AI 是辅助。
- 需要创建内容时，只能提交候选项。
- 候选项每次只提交一个，等待用户确认。
- Context 是理解来源，不是附件、背景材料或普通 source field。
- 如果用户要求保存 AI 生成的总结，先确认它是否代表用户自己的理解。
- 如果 Thought 没有 Context，要明确这是未追溯边界。
...
```ts
type RetrieveUnderstandingInput = {
  query: string;
  categoryIds?: string[];
...
type RetrieveUnderstandingResult = {
...
```ts
type ThoughtCaptureInput = {
  title?: string;
  body: string;
  categoryIds?: string[];
  sourceStatus: "grounded" | "ungrounded";
  sourceContext?: {
...
- `sourceStatus = grounded` 时必须带 `sourceContext`。
- `sourceStatus = ungrounded` 时 proposal UI 必须显示“未追溯来源”。
- 如果用户只是在让 AI 总结材料，Agent 不能直接把总结当 Thought；应该先问用户这是否代表他的理解。
- approval 后，后端用一次事务创建 Thought，并在有 `sourceContext` 时一起创建 Context。
- `context_create` 保留，用于给已有 Thought 增加来源。
- `thought_update` 保留，但 prompt 必须要求先读取现有 Thought 和 Context。
...
3. RED：写 prompt contract test，断言 runtime prompt 包含 `Thought`、`Context`、`用户是大脑`、`候选项`、`Context 是理解的根`。
4. GREEN：补齐 prompt。
...
- Tool card 文案更接近“查找相关理解 / 读取来源”。

改动：

- `thought_get` description 改为读取 Thought 及其来源 Context。
...
### Phase 3：新增 `retrieve_understanding` 作为主读工具

用户可见变化：

- 用户问“我关于 X 有哪些理解”时，Agent 能返回带来源的候选，而不是裸搜索结果。
- 如果没有命中，Agent 能说清楚是没有匹配，还是 query 太宽/太窄。

改动：

- 新增 `retrieve_understanding` Pi tool。
...
- 返回 `UnderstandingCandidate[]`：
  ...
  - boundary notes
  - retrieval trace
- System prompt 指导 Agent 优先使用 `retrieve_understanding`，只有需要精读时再 `thought_get` / `context_get`。

TDD：

1. RED：seed Thought + Context，调用 `retrieve_understanding("命中 Context 的词")`，期望返回父 Thought 和命中的 Context evidence。
2. GREEN：接入 `KnowledgeRetriever`。
3. RED：seed 只有 Thought、没有 Context 的记录，期望 candidate 带 boundary note。
4. GREEN：补 boundary note。
5. E2E：真实 AI 下让用户查询一个已 seed 的主题，断言出现 tool activity，最终回复完成，并且 UI 有可读的来源/候选展示。

退出条件：

- `retrieve_understanding` 覆盖普通查询路径。
- `search_all` 不再是 prompt 主推工具。
- 空结果有可解释 fallback。

### Phase 4：新增 `thought_capture` 候选写入工具

用户可见变化：

- 用户让 Agent 记录一条理解时，proposal card 会显示它是否有来源。
- 带来源的理解确认后，会同时创建 Thought 和 Context。
- 没有来源的理解会明确显示“未追溯来源”。

改动：

- 新增 `thought_capture` approval tool。
- approval 后用一次后端事务：
  - 创建 Thought。
  - 如果有 `sourceContext`，创建 Context。
- System prompt 规定：
  - 用户已经表达的理解可以整理成 candidate。
  - AI 自己总结出的内容只能作为候选表达，必须说明来源状态。
  - 缺少来源时优先追问；用户明确要先记录时，标记为 ungrounded。
- UI proposal card 展示：
  - Thought title/body。
  - source status。
  - source type/name/content preview。
  - proposal reason。

TDD：

1. RED：调用 `thought_capture` approval tool，pending event payload 包含 `sourceStatus`。
2. GREEN：注册 approval tool。
3. RED：approve grounded proposal 后，DB 里同时出现 Thought 和 Context。
4. GREEN：实现事务写入。
5. RED：approve ungrounded proposal 后，只创建 Thought，并在事件输出中保留 ungrounded 状态。
6. GREEN：补输出。
7. E2E：真实 AI 下输入“把这段来自某次 AI 对话的理解记录下来...”，断言出现 pending proposal；点击确认后，刷新仍能看到新 Thought 和 Context。

退出条件：

- 新写入路径不需要 Agent 连续调用 `thought_create` + `context_create`。
- 写入 proposal 能表达来源状态。
- 旧 `thought_create` 不再作为 prompt 主推工具。
...
1. RED：agent turn view 测试输入 `retrieve_understanding` event，期望 UI 显示“查找相关理解”。
2. GREEN：更新 tool display mapping。
3. RED：tool registry contract test，断言 prompt 主推工具包含 `retrieve_understanding`，不包含强制 `search_all`。
4. GREEN：收敛 registry / guidelines。
5. E2E：真实 AI 下跑三个 happy path：
   - 查询已有理解。
   - 记录带来源理解。
   - 给已有 Thought 补 Context。
...
- `retrieve_understanding` 对 seed DB 的 deterministic result。
- `thought_capture` approve 后的事务结果。
...
- `retrieve_understanding` 是 Agent 查询个人理解的主路径。
- `thought_capture` 是 Agent 创建新理解候选的主路径。
- 所有写入仍然走 approval。
