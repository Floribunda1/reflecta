...
└── 主区：画布工作区
     ├── 顶部工具栏：画布标题、+ 理解卡、+ 文本卡、撤销 / 重做、适应视图
     ├── X6 画布（无限画布、点状网格、平移缩放、参考线、框选多选、键盘）
     ├── 右侧理解库面板（搜索理解，拖入画布）
     └── 右侧理解详情面板（点击理解卡打开，复用 UnderstandingDetail，可关闭）
```
...
| F3  | 拖入理解卡          | 右侧面板搜索理解库 → 拖到画布落点（或点击加入画布中心）         | 落点位置正确、卡内显示标题与正文全文                 |
| F4  | 新建文本卡          | 工具栏按钮 / 双击空白处                                         | 创建后可编辑文本                                     |
...
| F8  | 连线标签            | 选中连线后在工具栏输入自由文本标签；双击连线可编辑              | 标签持久化、移动卡片时随动                           |
...
apps/electron/src/renderer/src/modules/understanding-canvas/
  ...
  canvas/
    ...
    CanvasToolbar.tsx            # 标题、+ 卡片、撤销 / 重做、缩放、标签输入
    LibraryPanel.tsx             # 理解库搜索 + DnD 拖入
