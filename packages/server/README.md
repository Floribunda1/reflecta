...
├─────────────────────────────────────────┤
│  BFF Core                               │
│  ├── thought-core  (Thought DB 操作)     │
│  ├── context-core  (Context DB 操作)     │
│  ├── category-core (Category DB 操作)    │
│  ├── search-core   (FTS 搜索操作)        │
│  ├── shared        (通用工具函数)        │
│  └── types         (共享类型定义)        │
├─────────────────────────────────────────┤
│  Core                                   │
...
| **BFF Core**  | 共享的底层数据库操作，被 electron / cli BFF 复用 | `bff/core/*-core.ts`                               |
...
- **BFF Core 复用**：将 electron / cli 中重复的 DB 操作、事务逻辑、FTS 操作抽象到 core，避免两边维护同样的 SQL。
...
import { ThoughtCore } from "@reflecta/server/bff/core";
