...
2. 在 GitHub 的 arm64 macOS runner 上构建 Sparkle 2.9.4 helper 和 Electron App。
...
“检查更新…”。发现更新时先显示 GitHub Release 的 changelog，用户确认后由 Sparkle
验证签名、优先下载 delta、替换 App 并重新启动。
...
- 本机构建 Sparkle helper 需要完整 Xcode。普通开发只需运行应用构建，正式打包交给
  GitHub Actions。
