功能: 全局行为
  ...
  场景: --quiet 抑制标准输出但不抑制错误
    假设 存在一条活跃 Thought，其 ID 为 THOUGHT_ID
    当 用户执行命令 "thought get THOUGHT_ID --quiet"
    那么 标准输出应为空
    并且 命令退出码应为 0

  场景: --quiet 模式下错误仍然输出到标准错误
    假设 存在一个数据库中不存在的 ID MISSING_ID
    当 用户执行命令 "thought get MISSING_ID --quiet"
    那么 标准输出应为空
  ...
  场景: 整数选项传入非数字值时报错
    当 用户执行命令 "thought list --limit not-a-number"
    那么 命令退出码应为 2
  ...
  场景: 嵌套帮助输出子命令列表
    当 用户执行命令 "reflecta thought --help"
    那么 标准输出应为一个 JSON 对象，列出 thought 下的所有子命令
