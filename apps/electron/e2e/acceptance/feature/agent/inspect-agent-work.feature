...
功能: 用户检查 Agent 的工作过程和结果
  ...
  @P1 @result @AG-RESULT-016
  场景: 用户从对话顶部查看已生成的知识条目
    假如对话中已经生成一条 Understanding「生成的理解」
    而且对话中已经生成一条 Context「生成的上下文」
    而且对话中已经生成一条 Domain「生成的领域」
    而且对话中已经生成一张画布「生成的画布」
    当用户打开该对话
    那么对话顶部应该显示一个带有 4 项数量的产出入口
    当用户打开产出入口
    那么列表应该显示「生成的理解」「生成的上下文」「生成的领域」和「生成的画布」
    而且四条内容应该分别以 Understanding、Context、Domain 和画布的类型图标区分
    当用户点击画布「生成的画布」
    那么应该以只读方式打开该画布

  @P1 @result @AG-RESULT-017
  场景: 用户从 Agent 回复查看 Agent 生成的分析画布
    假如对话中有一条 Agent 回复
    而且该回复包含 canvas_present 呈现的一张只读分析画布，标题为 CANVAS_TITLE
    当用户打开该对话
    那么回复中应该显示只读分析画布
    而且画布标题应该为 CANVAS_TITLE
    而且不应出现确认或拒绝操作
    而且这种只读呈现不进入对话顶部的产出入口
