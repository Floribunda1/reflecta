# 前端规范

## 技术栈

- react
- shadcn
- tailwindv4
- vite
- @tanstack/react-query
- jotai
- ahooks
- lodash-es
- date-fns

## 开发 Rules

- 在开发新模块/功能时，优先利用项目现有的三方依赖完成功能（见上面的技术栈），除非没有替代**不要自己造轮子**
  - 页面搭建优先使用 shadcn + tailwind
  - 通用的 hook 优先使用 ahooks
  - 多层级的状态管理优先使用 jotai、请求优先使用 @tanstack/react-query
  - 功能函数优先使用 lodash-es、date-fns

## React 性能相关

- 减少过多的 `useMemo` 优化，只有当确定某处为性能瓶颈时才考虑使用 `useMemo`

## 状态管理相关

- 请求数据、服务端缓存、异步加载状态优先使用 @tanstack/react-query。
- 多层级、跨组件共享的前端 UI 状态优先使用 jotai。
- 使用 jotai 时，优先让组件按需直接订阅 atom：
  - 只读状态使用 `useAtomValue`
  - 只写状态使用 `useSetAtom`
  - 读写同一个状态才使用 `useAtom`
- 不要把多个 jotai atoms 再聚合进 React Context 作为长期方案；这会让组件重新订阅一个粗粒度对象，抵消 jotai 按 atom 细粒度更新的收益。
- 所有跨层级传递 jotai 状态或 action 的地方，都必须直接使用 jotai atom、派生 atom 或 write-only action atom；不要用 React Context / `useContext` 转发 jotai 状态。
- 可以定义职责明确的派生 atom 或 write-only action atom，用于封装“选择分类并清空选中项”等复合状态操作。
- 不要创建一次性返回大量状态的聚合 hook；如果组件只需要其中一个字段，应直接订阅对应 atom。

## UI 相关

- 在设计页面的时候不要自己去造 **通用组件如 select 等**，能用 shadcn 就必须要用 shadcn
- 减少过多的 shadcn 本身的样式定制，尽量让整体的设计符合 shadcn 的 design system
- 优先使用项目下的 `use-drawer` 和 `use-modal` hook 而不是自己在该处重新写一遍 Dialog 或 Drawer
- 不允许对 shadcn 里的任何组件进行修改

## Review 检查顺序

在 review 前端模块时，按下面顺序逐项扫描，避免只看 UI 层而漏掉状态和 hook：

1. 技术栈复用：检查是否优先使用了项目已有依赖和组件能力，避免重复实现。
2. 通用 hook：检查是否手写了可由 ahooks 覆盖的 hook 或 hook 逻辑。
3. UI 组件：检查 select、menu、dialog、drawer 等通用组件是否优先使用 shadcn 或项目已有 `use-drawer`、`use-modal`。
4. React 性能：检查 `useMemo` 等优化是否有明确性能瓶颈依据。
5. 功能函数：检查日期、集合、对象处理是否优先使用 lodash-es、date-fns。
