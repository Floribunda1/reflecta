...
function ignoreAsyncG6Error(result: void | Promise<unknown>) {
  if (result && typeof result.catch === "function") {
    void result.catch(() => {
      // G6 can reject pending operations during route changes, HMR, or canvas teardown.
    });
  }
}
...
export function useGraphRenderer(
  ...
  const applyStates = (activeId: string | null) => {
    ...
    try {
      ...
      // G6 may receive a final state update while the canvas is being destroyed.
    }
  };
