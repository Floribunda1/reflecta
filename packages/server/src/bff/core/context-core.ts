...
import type { CreateContextInput, ReflectaDb, SourceType, UpdateContextInput } from "./types";

export async function listContextRows(
  db: ReflectaDb,
  thoughtId: string,
): Promise<Array<typeof contexts.$inferSelect>> {
  return db
    .select()
    .from(contexts)
...
export async function getContextRow(
  db: ReflectaDb,
  id: string,
): Promise<typeof contexts.$inferSelect | null> {
  const rows = await db
    .select()
    .from(contexts)
...
export async function deleteContext(db: ReflectaDb, id: string): Promise<void> {
  await db.transaction(async (tx) => {
    const rows = await tx
      .update(contexts)
...
export async function restoreContext(db: ReflectaDb, id: string): Promise<void> {
  await db.transaction(async (tx) => {
    const rows = await tx
      .update(contexts)
      .set({ deletedAt: null })
...
export async function permanentlyDeleteContext(db: ReflectaDb, id: string): Promise<void> {
  await db.transaction(async (tx) => {
...
export async function listTrashedContextRows(db: ReflectaDb): Promise<
  Array<{
    id: string;
    thoughtId: string;
