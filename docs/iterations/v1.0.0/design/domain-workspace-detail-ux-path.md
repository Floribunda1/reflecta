...
> 状态：Current
>
> 主题：Domain Workspace 右侧 Understanding Document 的阅读、Context追溯和双链跳转路径。
>
> 输入依据：
>
...
> 我当时形成的理解是什么？它从哪些Context长出来？它在正文中通过哪些双链连接到其他理解？

因此，Document 的设计重点不是把标题、正文、Context、关系都平铺出来，而是组织一条自然路径：

1. 先恢复理解本身。
2. 再感知Context。
3. 需要时打开单条Context。
4. 在正文中看到双链并可跳转。
5. 必要时直接修改正文、Context或领域归属。
...
Document 的第一视觉焦点必须是理解标题和正文。Context、关系、领域归属和操作入口都要从属于这个焦点。
...
- Context 内容大概是什么？
- 我是否需要打开完整Context重新读一遍？

Context必须可见，但不能默认抢走正文。它应该像Context和现场线索，而不是把 Document 变成资料阅读器。
...
历史样例中的Context长度说明，Context不能被当成一行 metadata：
...
单条理解的Context总长度可能很长。因此 Document 必须同时满足：

1. 正文不被长 Context淹没。
2. 用户能在正文下方感知Context存在。
3. 点击某条Context时，能阅读这一条完整Context。
4. 关闭Context后，能回到原理解现场。
...
用户读完正文
  -> 扫到 Context Trace
  -> 看到Context medium、Context 标题、字数和短预览
  -> 判断是否要打开完整Context
```
...
因此，点击某条Context 摘要，只能打开这一条Context的详情。它不应该进入“全部Context reader”，也不应该进入Context 管理后台。
...
完整Context只在用户点击某条Context 摘要后展示。它需要独立滚动空间，避免把正文推到不可见。
...
| -------------------- | -------------------- | -------------------------- | ---------------------- |
| 点开一条理解         | 看这条理解           | 显示标题和正文             | 先展示Context 管理界面 |
| 扫到Context 摘要     | 知道理解从哪里来     | 展示类型、名称、短 preview | 展开完整长文           |
| 点击某条Context 摘要 | 看这一条Context 全文 | 打开该Context 详情 overlay | 进入全部Context reader |
| 关闭Context 详情     | 回到刚才的理解       | 返回原 Document            | 丢失当前理解上下文     |
...
它不是全部Context 管理器，也不是新的页面主状态。它是从 Document 临时打开的一条Context 详情。
...
2. Context是否让用户感到“这个理解有根”，而不是“我被迫读资料”？
3. 点击某条Context后，系统是否只打开这一条Context？
4. 关闭Context后，用户是否自然回到原理解现场？
5. `[[双链]]` 是否作为正文的一部分自然出现？
6. 点击 `[[双链]]` 是否直接跳到目标理解？
7. 删除 `[[双链]]` 后，关系展示是否随正文变化？
8. 长 Context是否有独立滚动空间，而不是把正文推走？
9. 默认界面是否避免了Context 评分、关系类型、系统评分等未共识概念？

如果以上任何一条不成立，Document 就可能退回到了字段铺陈、Context后台或关系后台。
